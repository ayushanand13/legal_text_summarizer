...
class BertParent(object):
    ...
    def __init__(
        ...
        if torch.cuda.is_available():
            assert isinstance(gpu_id, int) and (
                0 <= gpu_id and gpu_id < torch.cuda.device_count()
    ...
    def create_matrix(
        ...
        return np.asarray(
            [
                np.squeeze(
                    self.extract_embeddings(t, hidden=hidden, reduce_option=reduce_option, hidden_concat=hidden_concat)
                    .data.cpu()
                    .numpy()
                )
                for t in content
